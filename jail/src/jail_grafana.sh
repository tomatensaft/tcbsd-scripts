...
#Print Header
print_header 'setup grafana in jail'
...
set_rc_remote() {  
    ...
    add_or_replace_in /${location_main=}/${jail_name}/etc/rc.conf 'grafana_enable=' '"YES"'
}
...
start_service() {
    ...
    jexec ${jail_name} service grafana start
    jexec ${jail_name} service grafana status
}
